...
public function dataTable(QueryBuilder $query): EloquentDataTable
{
    return (new EloquentDataTable($query))->setRowId('id')
        ->addColumn('images', function ($supplier) {
            $images = explode('|', $supplier->images);
            $html = '';
        ...
        $showUrl = route('supplier.show', $supplier->id);
        $id = '<span>'.$supplier->id.'</span>';
        if (!$supplier->deleted_at) {
            $id = '<a href="'.$showUrl.'">'.$supplier->id.'</a>';
        }
        return $id;
        })
        ->addColumn('action', function($supplier) {
            $editUrl = route('supplier.edit', $supplier->id);
            $csrf = csrf_field();

            if ($supplier->deleted_at) {
                $restoreUrl = route('supplier.restore', $supplier->id);
                $restoreButton = '<a href="'.$restoreUrl.'" class="btn btn-success">Restore</a>';

                $forceDeleteUrl = route('supplier.forceDelete', $supplier->id);
                ...
                $deleteUrl = route('supplier.destroy', $supplier->id);
...
public function getColumns(): array
{
    return [
        Column::make('id'),
        Column::make('name')->title('supplierName'),
        Column::make('email'),
        Column::make('images'),
        Column::make('action')
            ->exportable(false)
